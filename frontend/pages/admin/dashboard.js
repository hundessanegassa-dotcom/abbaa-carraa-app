...
export default function AdminDashboard() {
  ...
  async function verifyAgent(agentId, verified) {
    ...
    if (error) toast.error('Failed'); else { toast.success(`Agent ${verified ? 'approved' : 'rejected'}`); loadData(); }
  }
  ...
  async function verifyVendor(vendorId, verified) {
    ...
    if (error) toast.error('Failed'); else { toast.success(`Vendor ${verified ? 'approved' : 'rejected'}`); loadData(); }
  }

  async function updateUserRole(userId, newRole) {
    const { error } = await supabase.from('profiles').update({ role: newRole, user_type: newRole }).eq('id', userId);
    if (error) toast.error('Failed'); else { toast.success('User role updated'); loadData(); }
  }
  ...
  async function toggleFeaturedPool(poolId, isFeatured) {
    ...
    if (error) toast.error('Failed'); else { toast.success(`Pool ${!isFeatured ? 'featured' : 'removed'}`); loadData(); }
  }
      ...
      <div className="border-b bg-white sticky top-0 z-10 overflow-x-auto">
        <div className="container mx-auto px-4">
          <div className="flex gap-1 min-w-max">
            ...
            <button onClick={() => setActiveTab('all-pools')} className={`px-4 py-3 font-semibold ${activeTab === 'all-pools' ? 'border-b-2 border-red-600 text-red-600' : 'text-gray-500'}`}>🌊 All Pools</button>
            <button onClick={() => setActiveTab('approvals')} className={`px-4 py-3 font-semibold ${activeTab === 'approvals' ? 'border-b-2 border-red-600 text-red-600' : 'text-gray-500'}`}>📝 Approvals ({pendingAgents.length + pendingVendors.length})</button>
      ...
      <div className="container mx-auto px-4 py-8">
        {activeTab === 'overview' && (
          <div className="space-y-6">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div className="bg-white rounded-xl shadow-md p-6"><h3 className="font-bold mb-4">Financial Summary</h3><div className="space-y-2"><div className="flex justify-between"><span>Total Volume:</span><span className="font-bold">ETB {stats.total_volume.toLocaleString()}</span></div><div className="flex justify-between"><span>Charity Fund (2%):</span><span className="font-bold text-pink-600">ETB {Math.floor(stats.charity_total).toLocaleString()}</span></div></div></div>
              <div className="bg-white rounded-xl shadow-md p-6"><h3 className="font-bold mb-4">Admin's Personal Stats (20% Commission)</h3><div className="grid grid-cols-3 gap-3 text-center"><div><p className="text-2xl font-bold text-red-600">{myPools.length}</p><p className="text-xs">My Pools</p></div><div><p className="text-2xl font-bold text-green-600">{myPools.filter(p => p.status === 'active').length}</p><p className="text-xs">Active</p></div><div><p className="text-2xl font-bold text-yellow-600">ETB {myPools.reduce((sum, p) => sum + (p.target_amount * 0.20), 0).toLocaleString()}</p><p className="text-xs">Earned</p></div></div></div>
            </div>
          </div>
        )}
        ...
        {activeTab === 'approvals' && (
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-md p-4"><h3 className="font-bold mb-3">Pending Vendors ({pendingVendors.length})</h3>{pendingVendors.map(v => (<div key={v.id} className="flex justify-between items-center border-b py-2"><span>{v.profiles?.full_name} ({v.business_name})</span><div><button onClick={() => verifyVendor(v.id, true)} className="bg-green-600 text-white px-2 py-1 rounded text-sm mr-2">Approve</button><button onClick={() => verifyVendor(v.id, false)} className="bg-red-600 text-white px-2 py-1 rounded text-sm">Reject</button></div></div>))}</div>
          </div>
        )}
