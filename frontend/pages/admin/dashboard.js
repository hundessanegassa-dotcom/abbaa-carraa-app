...
// Import commission functions (now working since you created the file)
      ...
      <div className="border-b bg-white">
        <div className="container mx-auto px-4">
          <div className="flex flex-wrap gap-1">
            ...
            <button onClick={() => setActiveTab('my-pools')} className={`px-6 py-3 font-semibold transition ${activeTab === 'my-pools' ? 'border-b-2 border-red-600 text-red-600' : 'text-gray-500 hover:text-gray-700'}`}>🎯 My Pools (20% Commission)</button>
        ...
        {activeTab === 'overview' && (
          <div className="space-y-6">
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
              <div className="bg-white rounded-xl shadow-md p-4"><div className="text-2xl mb-2">👥</div><p className="text-2xl font-bold text-blue-600">{stats.total_users}</p><p className="text-xs text-gray-500">Total Users</p></div>
              <div className="bg-white rounded-xl shadow-md p-4"><div className="text-2xl mb-2">🤝</div><p className="text-2xl font-bold text-yellow-600">{stats.total_agents}</p><p className="text-xs text-gray-500">Agents</p></div>
              <div className="bg-white rounded-xl shadow-md p-4"><div className="text-2xl mb-2">🏭</div><p className="text-2xl font-bold text-purple-600">{stats.total_vendors}</p><p className="text-xs text-gray-500">Vendors</p></div>
              <div className="bg-white rounded-xl shadow-md p-4"><div className="text-2xl mb-2">🏢</div><p className="text-2xl font-bold text-cyan-600">{stats.total_organizations}</p><p className="text-xs text-gray-500">Organizations</p></div>
              <div className="bg-white rounded-xl shadow-md p-4"><div className="text-2xl mb-2">💰</div><p className="text-2xl font-bold text-green-600">ETB {Math.floor(stats.total_volume / 1000)}K</p><p className="text-xs text-gray-500">Total Volume</p></div>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-white rounded-xl shadow-md p-6"><h3 className="font-bold text-gray-500 mb-2">Prize Pools</h3><p className="text-3xl font-bold text-green-600">{stats.total_pools}</p><p className="text-sm text-gray-400 mt-1">{stats.active_pools} active | {stats.completed_pools} completed</p></div>
              <div className="bg-white rounded-xl shadow-md p-6"><h3 className="font-bold text-gray-500 mb-2">Commission Paid</h3><p className="text-3xl font-bold text-yellow-600">ETB {stats.total_commission_paid.toLocaleString()}</p><p className="text-sm text-gray-400 mt-1">{stats.pending_commission.toLocaleString()} ETB pending</p></div>
              <div className="bg-white rounded-xl shadow-md p-6"><h3 className="font-bold text-gray-500 mb-2">Admin Earnings</h3><p className="text-3xl font-bold text-red-600">ETB {myStats.total_commission.toLocaleString()}</p><p className="text-sm text-gray-400 mt-1">From your personal pools (20% commission)</p></div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <Link href="/admin/analytics" className="bg-gradient-to-r from-blue-500 to-indigo-500 rounded-xl p-4 text-white text-center hover:shadow-lg transition"><div className="text-2xl mb-1">📊</div><p className="font-semibold">Analytics</p><p className="text-xs opacity-90">View platform insights</p></Link>
          ...
          <div className="space-y-6">
            <div className="bg-gradient-to-r from-red-50 to-rose-50 rounded-xl p-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div className="bg-white rounded-lg p-3 text-center"><p className="text-2xl font-bold text-blue-600">ETB {myStats.total_raised.toLocaleString()}</p><p className="text-xs text-gray-500">Total Raised</p></div>
            ...
            <div className="bg-white rounded-xl shadow-md overflow-hidden">
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead className="bg-gray-50"><tr><th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Prize</th><th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Target</th><th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Raised</th><th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Your Commission</th><th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Status</th><th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Action</th></tr></thead>
                  <tbody className="divide-y divide-gray-200">
                    {myPools.length === 0 ? <tr><td colSpan="6" className="px-6 py-8 text-center text-gray-400">No pools created yet. <button onClick={createPool} className="text-red-600">Create your first pool →</button></td></tr> : myPools.map(pool => {
                      const commission = (pool.target_amount || 0) * 0.20;
                      return <tr key={pool.id} className="hover:bg-gray-50"><td className="px-6 py-4 font-medium">{pool.prize_name}</td><td className="px-6 py-4">ETB {(pool.target_amount || 0).toLocaleString()}</td><td className="px-6 py-4">ETB {(pool.current_amount || 0).toLocaleString()}</td><td className="px-6 py-4 font-semibold text-green-600">ETB {commission.toLocaleString()}</td><td className="px-6 py-4"><span className={`px-2 py-1 rounded-full text-xs ${pool.status === 'active' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>{pool.status}</span></td><td className="px-6 py-4"><Link href={`/pools/${pool.id}`} className="text-red-600 text-sm">View →</Link></td></tr>;
                    })}
                  </tbody>
                </table>
              </div>
        ...
        {activeTab === 'users' && (
          <div className="bg-white rounded-xl shadow-md overflow-hidden">
            <div className="overflow-x-auto">
              ...
              <tbody>{users.map(user => <tr key={user.id}><td className="px-6 py-4">{user.full_name}</td><td className="px-6 py-4">{user.email}</td><td className="px-6 py-4"><span className={`px-2 py-1 rounded-full text-xs ${user.role === 'admin' ? 'bg-red-100 text-red-800' : 'bg-gray-100 text-gray-800'}`}>{user.role || 'user'}</span></td><td className="px-6 py-4">{user.user_type || 'individual'}</td><td className="px-6 py-4">{user.role !== 'admin' && (<select onChange={(e) => updateUserRole(user.id, e.target.value)} className="text-sm border rounded px-2 py-1"><option value="user">User</option><option value="agent">Agent</option><option value="vendor">Vendor</option><option value="organization">Organization</option></select>)}</td></tr>)}</tbody></table>
            </div>
          </div>
        )}
        ...
        {activeTab === 'all-pools' && (
          <div className="bg-white rounded-xl shadow-md overflow-hidden">
            <div className="overflow-x-auto">
              ...
              <tbody>{allPools.map(pool => <tr key={pool.id}><td className="px-6 py-4 font-medium">{pool.prize_name}</td><td className="px-6 py-4">{pool.profiles?.full_name || 'Admin'}</td><td className="px-6 py-4">ETB {(pool.target_amount || 0).toLocaleString()}</td><td className="px-6 py-4"><span className={`px-2 py-1 rounded-full text-xs ${pool.status === 'active' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>{pool.status}</span></td><td className="px-6 py-4"><Link href={`/pools/${pool.id}`} className="text-red-600 text-sm">View →</Link></td></tr>)}</tbody></table>
            </div>
          </div>
        )}
      </div>
