      ...
      <div className="container mx-auto px-4 py-8">
        ...
        {activeTab === 'approvals' && (
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-md p-4"><h3 className="font-bold mb-3">🏭 Pending Vendors ({pendingVendors.length})</h3>{pendingVendors.map(v => (<div key={v.id} className="flex justify-between border-b py-2"><span>{v.business_name}<br/><span className="text-xs text-gray-500">{v.profiles?.email}</span></span><div><button onClick={() => verifyVendor(v.id, true)} className="bg-green-600 text-white px-3 py-1 rounded text-sm">Approve</button><button onClick={() => verifyVendor(v.id, false)} className="bg-red-600 text-white px-3 py-1 rounded text-sm ml-2">Reject</button></div></div>))}</div>
            <div className="bg-white rounded-xl shadow-md p-4"><h3 className="font-bold mb-3">🏢 Pending Organizations ({pendingOrganizations.length})</h3>{pendingOrganizations.map(o => (<div key={o.id} className="flex justify-between border-b py-2"><span>{o.business_name}<br/><span className="text-xs text-gray-500">{o.profiles?.email}</span></span><div><button onClick={() => verifyOrganization(o.id, true)} className="bg-green-600 text-white px-3 py-1 rounded text-sm">Approve</button><button onClick={() => verifyOrganization(o.id, false)} className="bg-red-600 text-white px-3 py-1 rounded text-sm ml-2">Reject</button></div></div>))}</div>
          </div>
        )}
        ...
        {activeTab === 'withdrawals' && (
          <div className="bg-white rounded-xl shadow-md overflow-hidden p-4">
            {withdrawalRequests.length === 0 ? <p className="text-center py-8 text-gray-400">No pending withdrawals</p> : withdrawalRequests.map(w => (<div key={w.id} className="flex justify-between items-center border-b py-3"><div><p className="font-medium">{w.profiles?.full_name}</p><p className="text-sm">ETB {w.amount?.toLocaleString()} - {w.commission_type}</p></div><div><button onClick={() => processWithdrawal(w.id, true)} className="bg-green-600 text-white px-3 py-1 rounded text-sm">Approve</button><button onClick={() => processWithdrawal(w.id, false)} className="bg-red-600 text-white px-3 py-1 rounded text-sm ml-2">Reject</button></div></div>))}
          </div>
        )}
