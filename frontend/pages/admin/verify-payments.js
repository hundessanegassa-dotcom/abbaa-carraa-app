// pages/admin/verify-payments.js
...
export default function VerifyPayments() {
  ...
  return (
    <>
      <Head>
        <title>Verify Payments - Admin | Abbaa Carraa</title>
      </Head>
          ...
          <div className="container mx-auto px-4">
            <div className="flex justify-between items-center flex-wrap gap-4">
              <div>
                ...
                <p className="text-gray-300 mt-1">Approve or reject payment proofs</p>
              </div>
              <Link href="/admin/dashboard" className="bg-white/20 hover:bg-white/30 px-4 py-2 rounded-lg transition">
                ← Back to Dashboard
              </Link>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-md overflow-hidden mb-6">
            <div className="flex border-b">
              <button
                ...
                className={`px-6 py-3 font-semibold transition ${activeTab === 'merkato' ? 'bg-yellow-50 text-yellow-700 border-b-2 border-yellow-500' : 'text-gray-500 hover:text-gray-700'}`}
              >
                🏪 Merkato VIP ({stats.merkatoPending})
              </button>
              <button
                onClick={() => { setActiveTab('city'); setSelectedCity('all'); setSelectedPoolType('all'); }}
                className={`px-6 py-3 font-semibold transition ${activeTab === 'city' ? 'bg-blue-50 text-blue-700 border-b-2 border-blue-500' : 'text-gray-500 hover:text-gray-700'}`}
              >
                🏙️ City VIP ({stats.cityPending})
              </button>
              <button
                onClick={() => { setActiveTab('regular'); setSelectedCity('all'); setSelectedPoolType('all'); }}
                className={`px-6 py-3 font-semibold transition ${activeTab === 'regular' ? 'bg-green-50 text-green-700 border-b-2 border-green-500' : 'text-gray-500 hover:text-gray-700'}`}
              >
