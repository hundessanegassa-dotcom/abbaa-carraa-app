...
export default function AgentDashboard() {
  ...
  const [selectedPoolForShare, setSelectedPoolForShare] = useState(null);
      ...
      <div className="container mx-auto px-4">
          ...
          <div>
            ...
            {agent && (
              <p className="text-gray-500 mt-1">
                Welcome back, {agent.business_name || 'Agent'}
              </p>
            )}
          </div>
          ...
          <div className="flex flex-wrap gap-4">
            {recentPools.slice(0, 3).map(pool => (
              <div key={pool.id} className="border rounded-lg p-4 text-center w-48">
                ...
                <button
                  onClick={() => setSelectedPoolForShare(selectedPoolForShare === pool.id ? null : pool.id)}
                  className="w-full bg-gray-600 text-white px-3 py-1 rounded-lg text-sm hover:bg-gray-700 mb-2"
                >
                  {selectedPoolForShare === pool.id ? 'Hide Link' : 'Show Share Link'}
                </button>
                {selectedPoolForShare === pool.id && (
        ...
        <div className="mt-8 bg-gradient-to-r from-blue-50 to-green-50 rounded-lg p-6">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            <div className="flex items-start gap-2"><span className="text-green-600">✓</span><p className="text-sm text-gray-700"><strong>Share your pools</strong> on social media and WhatsApp groups</p></div>
            <div className="flex items-start gap-2"><span className="text-green-600">✓</span><p className="text-sm text-gray-700"><strong>Offer discounts</strong> to non-winners to attract more participants</p></div>
            <div className="flex items-start gap-2"><span className="text-green-600">✓</span><p className="text-sm text-gray-700"><strong>Respond quickly</strong> to participant questions to build trust</p></div>
            <div className="flex items-start gap-2"><span className="text-green-600">✓</span><p className="text-sm text-gray-700"><strong>Create high-value prizes</strong> that attract more attention</p></div>
            <div className="flex items-start gap-2"><span className="text-green-600">✓</span><p className="text-sm text-gray-700"><strong>Add real images</strong> of your products (not just stock photos)</p></div>
            <div className="flex items-start gap-2"><span className="text-green-600">✓</span><p className="text-sm text-gray-700"><strong>Use share links</strong> to promote pools in print materials or shops</p></div>
          </div>
        </div>
      </div>
    </div>
