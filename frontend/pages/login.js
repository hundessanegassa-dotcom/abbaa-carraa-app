...
export default function Login() {
  ...
  useEffect(() => {
    const handleGoogleCallback = async () => {
      ...
      if (user && router.query.from === 'google') {
  ...
  const handleVerified = async () => {
    ...
    await redirectToDashboardAfterLogin();
  };
  ...
  const signInWithGoogle = async () => {
    ...
    const { error } = await supabase.auth.signInWithOAuth({
      provider: 'google',
      options: { 
        redirectTo: `${window.location.origin}/login?from=google`,
      },
    });
    if (error) {
      toast.error(error.message);
  ...
  return (
    <>
      <Head><title>Login - Digital ETA</title></Head>
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-green-50 to-blue-50 py-12 px-4">
        <div className="max-w-md w-full bg-white rounded-2xl shadow-xl p-8">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-500 mt-2">Login to your Digital ETA account</p>
          </div>
          
          {/* Google Login - Primary Option for Ethiopia */}
          <button 
            onClick={signInWithGoogle} 
            disabled={loading} 
            className="w-full bg-white border border-gray-300 text-gray-700 py-3 rounded-xl font-semibold hover:bg-gray-50 transition flex items-center justify-center gap-3 mb-4"
          >
          ...
          <div className="relative my-6">
            ...
            <div className="relative flex justify-center text-sm">
              <span className="px-3 bg-white text-gray-400">or use phone</span>
            </div>
          </div>
          ...
          <div className="bg-green-50 rounded-xl p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="font-bold text-green-800">Login with Voice Call</h3>
              <span className="bg-green-200 text-green-800 text-xs px-2 py-0.5 rounded-full ml-auto">Recommended for Ethiopia</span>
            </div>
            <form onSubmit={handlePhoneSubmit} className="space-y-4">
              <div>
                ...
                <p className="text-xs text-gray-400 mt-1">
                  📞 You'll receive a voice call with OTP (SMS may be blocked)
                </p>
              </div>
              <button 
                type="submit" 
              ...
              >
                {loading ? 'Processing...' : 'Continue with Voice Call →'}
              </button>
            </form>
          </div>
          
          <div className="mt-8 text-center">
            <p className="text-gray-500 text-sm">Don't have an account? <Link href="/register" className="text-green-600 font-semibold">Create Account</Link></p>
          </div>
          
          <div className="mt-6 text-center text-xs text-gray-400">
            <p>💚 2% of income supports kidney & heart disease patients</p>
          </div>
        </div>
      </div>
    </>
