...
export default function BecomeOrganization() {
  ...
  async function checkUser() {
    ...
    // Check if already an organization organizer
  ...
  async function handleSubmit(e) {
    ...
    if (error) {
      ...
      toast.success('Organization application submitted! We will review it soon.');
            ...
            <div className="p-6 border-b">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div className="bg-blue-50 rounded-xl p-3 text-center">
                  ...
                  <p className="text-xs text-gray-500">On every pool</p>
                </div>
                <div className="bg-blue-50 rounded-xl p-3 text-center">
                  <div className="text-2xl mb-1">👥</div>
                  <p className="font-semibold">Member Management</p>
                  <p className="text-xs text-gray-500">Add & approve members</p>
                </div>
                <div className="bg-blue-50 rounded-xl p-3 text-center">
                  <div className="text-2xl mb-1">🔒</div>
                  <p className="font-semibold">Private Pools</p>
                  <p className="text-xs text-gray-500">Members-only access</p>
                </div>
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" value={formData.organization_name} onChange={(e) => setFormData({...formData, organization_name: e.target.value})} className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500" required />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Organization Type *</label>
                  <select value={formData.organization_type} onChange={(e) => setFormData({...formData, organization_type: e.target.value})} className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500" required>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" value={formData.registration_number} onChange={(e) => setFormData({...formData, registration_number: e.target.value})} className="w-full px-4 py-2 border rounded-lg" placeholder="License / Registration No" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">TIN (Tax ID)</label>
                  <input type="text" value={formData.tin} onChange={(e) => setFormData({...formData, tin: e.target.value})} className="w-full px-4 py-2 border rounded-lg" placeholder="For payouts above 10,000 ETB" />
                </div>
              </div>
              ...
              <div>
                ...
                <input type="url" value={formData.website} onChange={(e) => setFormData({...formData, website: e.target.value})} className="w-full px-4 py-2 border rounded-lg" placeholder="https://..." />
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">About Your Organization</label>
                <textarea rows="3" value={formData.description} onChange={(e) => setFormData({...formData, description: e.target.value})} className="w-full px-4 py-2 border rounded-lg" placeholder="Tell us about your organization and how you plan to engage members..." />
              </div>
