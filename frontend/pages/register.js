          ...
          <div className="max-w-md w-full bg-white rounded-2xl shadow-xl p-8">
            ...
            <div className="bg-green-50 rounded-xl p-4 mb-6">
              ...
              <form onSubmit={handlePhoneSubmit} className="space-y-4">
                ...
                <div><label className="block text-gray-700 mb-2 text-sm">Phone Number</label><div className="flex"><span className="inline-flex items-center px-4 rounded-l-xl border border-r-0 border-gray-300 bg-gray-50 text-gray-500">+251</span><input type="tel" required value={phone} onChange={(e) => setPhone(e.target.value)} className="flex-1 px-4 py-3 border rounded-r-xl focus:ring-2 focus:ring-green-500" placeholder="912345678" /></div><p className="text-xs text-gray-500 mt-1">We'll send a verification code via SMS or voice call</p></div>
                <button type="submit" disabled={loading} className="w-full bg-green-600 text-white py-3 rounded-xl font-semibold hover:bg-green-700 transition">{loading ? 'Sending...' : 'Continue →'}</button>
              </form>
            </div>
