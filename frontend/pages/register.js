...
export default function Register() {
  ...
  const roles = [
    { 
      ...
      name: 'Individual', 
      icon: '👤', 
      description: 'Join existing pools and win amazing prizes',
    ...
    { 
      ...
      name: 'Agent', 
      icon: '🤝', 
      description: 'Create prize pools and earn 10% commission',
    ...
    { 
      ...
      name: 'Vendor', 
      icon: '🏭', 
      description: 'List your products as prizes and reach customers',
  ...
  const sendOTP = async (e) => {
    ...
    // For testing – use fixed OTP since SMS is blocked in Ethiopia
    // In production, you'd use WhatsApp or Voice OTP
    const testOTP = Math.floor(100000 + Math.random() * 900000).toString();
    console.log('📱 Test OTP for', formattedPhone, ':', testOTP);
    
    // Store OTP in session for verification
    sessionStorage.setItem('test_otp', testOTP);
    sessionStorage.setItem('test_phone', formattedPhone);
    
    toast.success(`Test OTP sent to console! Code: ${testOTP}`);
    setStep('otp');
    setLoading(false);
    setCountdown(60);
  ...
  const verifyAndRegister = async (e) => {
    ...
    const formattedPhone = sessionStorage.getItem('test_phone');
  ...
  if (step === 'role') {
    return (
      <>
        <Head>
          <title>Join Abbaa Carraa</title>
          <meta name="description" content="Choose how you want to participate" />
        </Head>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
              {roles.map((role) => (
                <button
                  ...
                  className={`bg-gradient-to-r ${role.color} rounded-xl p-6 text-white text-left hover:shadow-xl transition-all transform hover:-translate-y-1 ${
                    selectedRole === role.id ? 'ring-4 ring-offset-2 ring-green-400' : ''
                  }`}
                >
              ...
              <form onSubmit={sendOTP} className="space-y-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    We'll send a verification code (test mode - check console)
                  </p>
                </div>
      ...
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-green-50 to-blue-50 py-12 px-4">
        <div className="max-w-md w-full bg-white rounded-2xl shadow-xl p-8">
          <div className="text-center mb-8">
            ...
            <p className="text-xs text-gray-400 mt-2">Check browser console for the code (SMS test mode)</p>
          </div>
