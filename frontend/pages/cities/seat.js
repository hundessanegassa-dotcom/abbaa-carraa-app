// pages/cities/seat.js - City VIP daily/weekly/monthly seat booking
...
export default function CitySeat() {
  ...
  const cityDisplayName = city ? decodeURIComponent(city).replace(/-/g, ' ') : 'City';
  ...
  const selectPool = poolId => {
    ...
    if (city) router.replace(`/cities/seat?city=${city}&type=${poolId}`, undefined, { shallow: true });
  };
  ...
  return (
    <>
      <Head>
        <title>{cityDisplayName} VIP - Select Seat | Abbaa Carraa</title>
      </Head>
      <div className="min-h-screen bg-gray-50 py-6 pb-32">
        <div className="container mx-auto px-4 max-w-6xl">
          ...
          <div className="bg-white rounded-2xl border p-5 mb-6 text-center">
            ...
            <h1 className="text-2xl font-bold">{cityDisplayName} VIP</h1>
              ...
              return (
                ...
                >
                  <div className="text-lg font-bold">
                    {option.icon} {language === 'am' ? option.labelAm : option.labelEn}
                  </div>
          ...
          {showCheckout && (
            <SeatCheckout
              ...
              poolName={`${cityDisplayName} VIP - ${language === 'am' ? poolInfo.labelAm : poolInfo.labelEn}`}
              user={user}
              language={language}
