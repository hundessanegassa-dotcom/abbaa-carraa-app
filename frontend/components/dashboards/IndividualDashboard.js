        ...
        <div className="bg-white rounded-2xl shadow-md p-6 hover:shadow-lg transition">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-green-600">ETB {totalContributions.toLocaleString()}</p>
            </div>
                      ...
                      <div key={entry.id} className="border rounded-xl p-4 hover:shadow-md transition">
                        <div className="flex justify-between items-start mb-2">
                          <div>
                            ...
                            <p className="text-sm text-gray-500">Contributed: ETB {entry.amount.toLocaleString()}</p>
                          </div>
            ...
            <div className="bg-white rounded-2xl shadow-md overflow-hidden">
              ...
              <div className="p-6">
                <div className="space-y-3">
                  {recentWins.map(win => (
                    <div key={win.id} className="flex items-center justify-between p-3 bg-yellow-50 rounded-xl">
                      <div>
                        <p className="font-semibold text-gray-800">{win.prize_name}</p>
                        <p className="text-sm text-gray-500">Won: {new Date(win.completed_at).toLocaleDateString()}</p>
                      </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-gradient-to-r from-green-500 to-teal-500 rounded-2xl p-6 text-white">
            ...
            <ul className="space-y-2 text-sm">
              <li>🔍 Browse prize pools</li>
              <li>💰 Contribute small amount</li>
              <li>🎲 Fair random draw</li>
              <li>🏆 Win amazing prizes!</li>
            </ul>
            <Link href="/listings" className="inline-block mt-4 bg-white text-green-600 px-4 py-2 rounded-full text-sm font-semibold hover:bg-gray-100">
              Start Playing →
            </Link>
          </div>
          
          <div className="bg-gradient-to-r from-red-500 to-pink-500 rounded-2xl p-6 text-white">
            <h3 className="text-xl font-bold mb-2">❤️ Making a Difference</h3>
            <p className="text-sm opacity-95 mb-3">2% of all income supports Ethiopians fighting kidney & heart disease.</p>
            <div className="flex items-center gap-2 text-sm">
              <span>💚</span>
              <span>Every contribution saves lives</span>
            </div>
          </div>
        </div>
      </div>
